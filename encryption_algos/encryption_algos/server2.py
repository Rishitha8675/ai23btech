...
Encrypted_AES_Key=connection.recv(128)
message_from_client=receive_message(connection)
print(message_from_client)
Encrypted_AES_iv=connection.recv(128)


AES_key=rsa.decrypt(Encrypted_AES_Key,private_key)
AES_iv=rsa.decrypt(Encrypted_AES_iv,private_key)
...
progress =tqdm.tqdm(unit="B",unit_scale=True,colour='red',unit_divisor=1000,total=int(file_size))
